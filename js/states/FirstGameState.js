...
SpaceShooter.FirstGameState = {
    ...
    create:function ()
    {
        ...
        this.game.time.events.add(1000, this.launchGreenEnemy(this.game));
        ...
        //  Shields stat
        shields = this.add.bitmapText(this.game.world.width - 250, 10, 'spacefont', '' + player.health + '%', 50);
        ...
        //  Game over text
        gameOver = this.add.bitmapText(this.game.world.centerX, this.game.world.centerY, 'spacefont', 'GAME OVER!', 110);
    ...
    update:function () {
        ...
        if (this.input.x < this.width - 20 &&
            this.input.x > 20 &&
            this.input.y > 20 &&
            this.input.y < this.height - 20) {
            var minDist = 200;
    ...
    launchGreenEnemy: function (game) {
        var ENEMY_SPEED = 300;

        this.enemy = greenEnemies.getFirstExists(false);
        if (enemy) {
            this.enemy.reset(game.rnd.integerInRange(0, game.width), -20);
            this.enemy.body.velocity.x = game.rnd.integerInRange(-300, 300);
            this.enemy.body.velocity.y = ENEMY_SPEED;
            this.enemy.body.drag.x = 100;

            this.enemy.trail.start(false, 800, 1);
        ...
        greenEnemyLaunchTimer = game.time.events.add(game.rnd.integerInRange(greenEnemySpacing, greenEnemySpacing + 1000), launchGreenEnemy(game));
    },
    ...
    restart:function(game) {
        ...
        game.time.events.add(1000, launchGreenEnemy(game));
