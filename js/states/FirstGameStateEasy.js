...
SpaceShooter.FirstGameStateEasy = {
    ...
    create:function ()
    {
        ...
        boss.finishOff = function () {
            if (!boss.dying) {
                ...
                this.game.time.events.add(1000, function () {
                    ...
                    youWin.visible = true;
                    youWin.alpha = 0;
                    var fadeInYouWin = this.SpaceShooter.FirstGameStateEasy.add.tween(youWin);
                    fadeInYouWin.to({alpha: 1}, 1000, Phaser.Easing.Quintic.Out);
                    fadeInYouWin.onComplete.add(setResetHandlers);
                    fadeInYouWin.start();
                    greenEnemies.callAll('kill');
                    this.SpaceShooter.FirstGameStateEasy.game.time.events.remove(greenEnemyLaunchTimer);
                    blueEnemies.callAll('kill');
                    blueEnemyBullets.callAll('kill');
                    this.SpaceShooter.FirstGameStateEasy.game.time.events.remove(blueEnemyLaunchTimer);
                    boss.kill();
                    booster.kill();
                    this.SpaceShooter.FirstGameStateEasy.game.time.events.remove(bossLaunchTimer);

                    blueEnemies.callAll('kill');
                    this.SpaceShooter.FirstGameStateEasy.game.time.events.remove(blueEnemyLaunchTimer);

                    function setResetHandlers() {
                        tapRestart = this.SpaceShooter.FirstGameStateEasy.input.onTap.addOnce(_restart, this);
                        spaceRestart = fireButton.onDown.addOnce(_restart, this);
                        function _restart() {
                            tapRestart.detach();
                            spaceRestart.detach();
                            this.SpaceShooter.FirstGameStateEasy.restart(this.SpaceShooter.FirstGameStateEasy.game);
                        }
                    }
                });
        ...
        booster.forEach(function (p) {
            ...
            p.anchor.x = this.SpaceShooter.FirstGameStateEasy.rnd.pick([1, -1]) * 0.95 + 0.5;
            p.anchor.y = 0.75;
        });
    ...
    createGreenEnemies:function(){
        ...
        greenEnemies.forEach(function (enemy) {
            this.SpaceShooter.FirstGameStateEasy.addEnemyEmitterTrail(enemy);
    ...
    hitEnemy:function(enemy, bullet) {
        ...
        if (!bossLaunched && score > 15000) {
            ...
            this.time.events.add(2000, function () {
                bossLaunched = true;
                this.SpaceShooter.FirstGameStateEasy.launchBoss();
            });
        }
